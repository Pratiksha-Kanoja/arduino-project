...
#define durationTime 30000 //Define the time it remains in the open position of the door lock (miliseconds) 
#define servoOpen 0 //Open motor 
#define servoClose 180 //Close motor 
...
serial port finger.begin(57600);

if (finger.verifyPassword()) 
{ Serial.println("Found fingerprint sensor!"); 
} else { Serial.println("Did not find fingerprint sensor :("); while (1); } Serial.println("Waiting for valid finger..."); }

void loop() // run over and over again 

{ getFingerprintIDez(); delay(1000); //don't need to run this at full speed. }
...
// servo1.write(servoMin); //If the fingerprint is correct open the door lock // delay(durationTime); //Keep the lock open for the defined duration // servo1.write(servoMax); //take the lock OFF again
